...
const accuacyTag = document.querySelector(".accuracy span")
console.log(accuacyTag)

let timer;
let maxTime = 60;
let timeLeft = maxTime;
let charIndex = mistakes = correct = isTyping = totalChar= 0;
...
function initTimer() {
    if (timeLeft > 0) {
        ...
        let wpm = Math.round(((charIndex - mistakes) / 5) / (maxTime - timeLeft) * 60);
        wpmTag.innerText = wpm;
        accuacyTag.innerText = Math.floor(((correct/charIndex)*100));
    } else {
        clearInterval(timer);
    }
}
